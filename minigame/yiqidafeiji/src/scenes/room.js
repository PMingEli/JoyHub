...
const emptyUser = {
    nickname: '点击邀请好友',
    headimg: "yiqidafeiji/images/avatar_default.png",
    isEmpty : true,
    isReady : false,
}
...
export default class Room extends PIXI.Container {
    ...
    appendBackBtn() {
        const back = createBtn({
            img   : 'yiqidafeiji/images/goBack.png',
            x     : 104,
            y     : 68,
            onclick: () => {
                wx.showModal({
    ...
    appendOpBtn(member) {
        ...
        let getReady = createBtn({
            img : 'yiqidafeiji/images/getReady.png',
        ...
        let start = createBtn({
            img : 'yiqidafeiji/images/start.png',
            x   : config.GAME_WIDTH / 2 + 159,
            y   : config.GAME_HEIGHT - 160,
            onclick: () => {
                if ( !this.allReady ) {
    ...
    createOneUser(options) {
        ...
        if ( role === config.roleMap.owner ) {
            const host = new PIXI.Sprite.from("yiqidafeiji/images/hosticon.png");
            host.scale.set(.8);
            host.y = -30;
            user.addChild(host);
        }
        ...
        if ( isReady && !databus.matchPattern ) {
            const ready = new PIXI.Sprite.from('yiqidafeiji/images/iconready.png');
            ready.width  = 40;
            ready.height = 40;
            ready.x = user.width;
            user.addChild(ready);
